...
const tally_host = process.env.TALLY_HOST || 'localhost'; // default to localhost XML server of Tally
...
export async function queryCollection(targetCollection: string, lstFields: string[], lstFilters: Map<string, string>, targetCompany?: string, fromDate?: Date, toDate?: Date): Promise<any[]> {
    ...
    try {
        ...
        // --- CACHING LOGIC ---
        // Do not cache transactional data like Bills or Vouchers
        let cacheKey = '';
        const canCache = targetCollection !== 'Bill' && targetCollection !== 'Voucher';
        if (canCache) {
            const filterStr = lstFilters ? JSON.stringify(Array.from(lstFilters.entries())) : '';
            cacheKey = `${targetCollection}_${lstFields.join(',')}_${filterStr}_${targetCompany}_${fromDate}_${toDate}`;
        ...
        if (canCache && cacheKey) {
            cacheStore.set(cacheKey, { data: retval, timestamp: Date.now() });
        }
